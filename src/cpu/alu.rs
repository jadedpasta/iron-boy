...
impl Cpu {
    fn alu(&mut self, src: Operand8, f: impl FnOnce(&mut u8, u8) -> u8, mem: &Memory) {
        let src = self.read_operand(src, mem);
        self.regs[Reg8::F] = f(&mut self.regs[Reg8::A], src);
    }

    pub(super) fn adc(&mut self, src: Operand8, mem: &Memory) {
    ...
    pub(super) fn add(&mut self, src: Operand8, mem: &Memory) {
    ...
    pub(super) fn sbc(&mut self, src: Operand8, mem: &Memory) {
        let curr_carry = self.regs.get_flag(Flag::CARRY);
        let f = |a: &mut u8, src: u8| -> u8 {
    ...
    pub(super) fn sub(&mut self, src: Operand8, mem: &Memory) {
        fn f(a: &mut u8, src: u8) -> u8 {
            let (borrow, half_borrow) = sub_impl(a, src);
            Flag::zero(*a == 0) | Flag::carry(borrow) | Flag::half_carry(half_borrow) | Flag::SUB
        }
    ...
    pub(super) fn cp(&mut self, src: Operand8, mem: &Memory) {
        let a = self.regs[Reg8::A];
        self.sub(src, mem);
        self.regs[Reg8::A] = a;
    }

    pub(super) fn and(&mut self, src: Operand8, mem: &Memory) {
        fn f(a: &mut u8, src: u8) -> u8 {
            *a &= src;
    ...
    pub(super) fn or(&mut self, src: Operand8, mem: &Memory) {
        fn f(a: &mut u8, src: u8) -> u8 {
            *a |= src;
            Flag::zero(*a == 0)
        }
    ...
    pub(super) fn xor(&mut self, src: Operand8, mem: &Memory) {
        fn f(a: &mut u8, src: u8) -> u8 {
            *a ^= src;
            Flag::zero(*a == 0)
        }
    ...
    pub(super) fn inc(&mut self, var: Var8, mem: &mut Memory) {
        let val = self.read_var(var, mem).wrapping_add(1);
        self.write_var(var, val, mem);
        self.regs.set_flags(Flag::HALF_CARRY, val & 0xf == 0);
        self.regs.set_flags(Flag::ZERO, val == 0);
    }

    pub(super) fn dec(&mut self, var: Var8, mem: &mut Memory) {
        let val = self.read_var(var, mem).wrapping_sub(1);
        self.write_var(var, val, mem);
    ...
    pub(super) fn bit(&mut self, bit: u8, var: Var8, mem: &Memory) {
    ...
    pub(super) fn res(&mut self, bit: u8, var: Var8, mem: &mut Memory) {
        let mut val = self.read_var(var, mem);
        val &= !(1 << bit);
        self.write_var(var, val, mem);
    }

    pub(super) fn set(&mut self, bit: u8, var: Var8, mem: &mut Memory) {
        let mut val = self.read_var(var, mem);
        val |= 1 << bit;
        self.write_var(var, val, mem);
    }

    fn alu_var(&mut self, var: Var8, f: impl FnOnce(&mut u8) -> u8, mem: &mut Memory) {
        let mut val = self.read_var(var, mem);
        self.regs[Reg8::F] = f(&mut val);
        self.write_var(var, val, mem);
    }

    pub(super) fn rl(&mut self, var: Var8, mem: &mut Memory) {
    ...
    pub(super) fn rla(&mut self, mem: &mut Memory) {
        self.rl(Var8::Reg(Reg8::A), mem);
        self.regs.set_flags(Flag::ZERO, false);
    }

    pub(super) fn rlc(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
    ...
    pub(super) fn rlca(&mut self, mem: &mut Memory) {
        self.rlc(Var8::Reg(Reg8::A), mem);
        self.regs.set_flags(Flag::ZERO, false);
    }

    pub(super) fn rr(&mut self, var: Var8, mem: &mut Memory) {
        let carry = self.regs.get_flag(Flag::CARRY) as u8;
        let f = |var: &mut u8| {
    ...
    pub(super) fn rra(&mut self, mem: &mut Memory) {
        self.rr(Var8::Reg(Reg8::A), mem);
        self.regs.set_flags(Flag::ZERO, false);
    }

    pub(super) fn rrc(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
    ...
    pub(super) fn rrca(&mut self, mem: &mut Memory) {
        self.rrc(Var8::Reg(Reg8::A), mem);
        self.regs.set_flags(Flag::ZERO, false);
    }

    pub(super) fn sla(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
            let carry_out = Flag::carry(*var >> 7 != 0);
            *var <<= 1;
            Flag::zero(*var == 0) | carry_out
    ...
    pub(super) fn sra(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
            let carry_out = Flag::carry(*var & 0x1 != 0);
    ...
    pub(super) fn srl(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
            let carry_out = Flag::carry(*var & 0x1 != 0);
            *var >>= 1;
            Flag::zero(*var == 0) | carry_out
    ...
    pub(super) fn swap(&mut self, var: Var8, mem: &mut Memory) {
        let f = |var: &mut u8| {
            *var = (*var >> 4) | (*var << 4);
            Flag::zero(*var == 0)
        };
