...
const fn branch(instruction: Instruction, cycles: usize, branch_cycles: usize) -> InstructionEntry {
    InstructionEntry {
        instruction,
        cycles,
        branch_cycles,
...
const OP_TABLE: [InstructionEntry; 0x100] = [
    ...
    new(Rra, 1),                                                // 0x1f
    branch(Jr(Some(Test::Nz)), 3, 2),                           // 0x20
    ...
    new(Daa, 1),                                                // 0x27
    branch(Jr(Some(Test::Z)), 3, 2),                            // 0x28
    ...
    new(Cpl, 1),                                                // 0x2f
    branch(Jr(Some(Test::Nc)), 3, 2),                           // 0x30
    ...
    new(Scf, 1),                                                // 0x37
    branch(Jr(Some(Test::C)), 3, 2),                            // 0x38
    ...
    branch(Ret(Some(Test::Nz)), 5, 2),                          // 0xc0
    new(Pop(Reg16::BC), 3),                                     // 0xc1
    branch(Jp(Some(Test::Nz)), 4, 3),                           // 0xc2
    new(Jp(None), 4),                                           // 0xc3
    branch(Call(Some(Test::Nz)), 6, 3),                         // 0xc4
    new(Push(Reg16::BC), 4),                                    // 0xc5
    new(Add(Operand8::Imm), 2),                                 // 0xc6
    new(Rst(0x00), 4),                                          // 0xc7
    branch(Ret(Some(Test::Z)), 5, 2),                           // 0xc8
    new(Ret(None), 4),                                          // 0xc9
    branch(Jp(Some(Test::Z)), 4, 3),                            // 0xca
    new(Illegal, 1),                                            // 0xcb
    branch(Call(Some(Test::Z)), 6, 3),                          // 0xcc
    new(Call(None), 6),                                         // 0xcd
    new(Adc(Operand8::Imm), 2),                                 // 0xce
    new(Rst(0x08), 4),                                          // 0xcf
    branch(Ret(Some(Test::Nc)), 5, 2),                          // 0xd0
    new(Pop(Reg16::DE), 3),                                     // 0xd1
    branch(Jp(Some(Test::Nc)), 4, 3),                           // 0xd2
    new(Illegal, 1),                                            // 0xd3
    branch(Call(Some(Test::Nc)), 6, 3),                         // 0xd4
    new(Push(Reg16::DE), 4),                                    // 0xd5
    new(Sub(Operand8::Imm), 2),                                 // 0xd6
    new(Rst(0x10), 4),                                          // 0xd7
    branch(Ret(Some(Test::C)), 5, 2),                           // 0xd8
    new(Reti, 4),                                               // 0xd9
    branch(Jp(Some(Test::C)), 4, 3),                            // 0xda
    new(Illegal, 1),                                            // 0xdb
    branch(Call(Some(Test::C)), 6, 3),                          // 0xdc
    new(Illegal, 1),                                            // 0xdd
